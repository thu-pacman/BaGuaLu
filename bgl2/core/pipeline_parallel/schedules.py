...
from bgl2.core.models.gpt import gpt_activation as activation_modeling
...
_PP_FAKE_FORWARD_BACKWARD_DONE = False


class _SyntheticBatchIterator:
    """Infinite iterator that yields lightweight GPT-style fake batches."""

    def __init__(self):
        args = get_args()
        self.micro_batch_size = args.micro_batch_size
        self.seq_length = args.seq_length
        self.create_attention_mask = getattr(args, "create_attention_mask_in_dataloader", True)
        self.reset_attention_mask = getattr(args, "reset_attention_mask", False)
        self._batch = self._build_batch()

    def _build_batch(self):
        tokens = torch.zeros((self.micro_batch_size, self.seq_length), dtype=torch.int64)
        labels = torch.zeros_like(tokens)
        loss_mask = torch.ones((self.micro_batch_size, self.seq_length), dtype=torch.float32)
        position_ids = (
            torch.arange(self.seq_length, dtype=torch.int64)
            .unsqueeze(0)
            .expand(self.micro_batch_size, -1)
            .contiguous()
        )
        if self.create_attention_mask:
            mask_batch = self.micro_batch_size if self.reset_attention_mask else 1
            attention_mask = torch.tril(
                torch.ones((mask_batch, self.seq_length, self.seq_length), dtype=torch.float32)
            ).view(mask_batch, 1, self.seq_length, self.seq_length)
            attention_mask = (attention_mask < 0.5).contiguous()
            if self.reset_attention_mask:
                attention_mask = attention_mask.expand(
                    self.micro_batch_size, 1, self.seq_length, self.seq_length
                ).contiguous()
        else:
            attention_mask = None
        return {
...
def _build_synthetic_data_iterators_like(data_iterator):
    if isinstance(data_iterator, list):
        return [_SyntheticBatchIterator() for _ in range(len(data_iterator))]
    return _SyntheticBatchIterator()


def _build_local_fake_pp_input_tensor(tensor_shape, model_chunk, requires_grad=False):
    """Build local dummy activation tensor for a compute-only fake PP pass."""
    try:
        ref_param = next(model_chunk.parameters())
...
def _build_fake_output_grad_like(output_tensor):
    """Build a dummy grad tensor matching the fake forward output structure."""
    if output_tensor is None:
        return None
    if isinstance(output_tensor, list):
        return [_build_fake_output_grad_like(tensor) for tensor in output_tensor]
    if isinstance(output_tensor, tuple):
        return tuple(_build_fake_output_grad_like(tensor) for tensor in output_tensor)
    return torch.ones_like(output_tensor)


def _deallocate_output_tensor_tree(output_tensor, deallocate_pipeline_outputs):
    if output_tensor is None:
        return
    if isinstance(output_tensor, (list, tuple)):
        for tensor in output_tensor:
            _deallocate_output_tensor_tree(tensor, deallocate_pipeline_outputs)
        return
    deallocate_output_tensor(output_tensor, deallocate_pipeline_outputs)


def _clone_cuda_rng_tracker_states(states):
    return {
        name: state.clone() if torch.is_tensor(state) else state for name, state in states.items()
    }


def _set_model_chunks_training(model_chunks, training):
    previous_modes = []
    for model_chunk in model_chunks:
        previous_modes.append([(module, module.training) for module in model_chunk.modules()])
        model_chunk.train(training)
    return previous_modes


def _restore_model_chunks_training(model_chunks, previous_modes):
    for module_modes in previous_modes:
        for module, previous_mode in module_modes:
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    def should_offload_forward(virtual_microbatch_id):
        if virtual_microbatch_id is None or forward_only:
            return False
        return not (
            pipeline_parallel_rank == pipeline_parallel_size - 1
            and get_model_chunk_id(virtual_microbatch_id, forward=True) == num_model_chunks - 1
        )

    def should_reload_backward(virtual_microbatch_id):
        if virtual_microbatch_id is None or forward_only:
            return False
        return not (
            pipeline_parallel_rank == pipeline_parallel_size - 1
            and get_model_chunk_id(virtual_microbatch_id, forward=False) == num_model_chunks - 1
        )

    def get_next_reload_virtual_microbatch_id(f_virtual_microbatch_id, b_virtual_microbatch_id):
        if forward_only:
            return None
        if b_virtual_microbatch_id is not None:
            reload_virtual_microbatch_id = b_virtual_microbatch_id + 1
        elif f_virtual_microbatch_id == num_warmup_microbatches - 1:
            reload_virtual_microbatch_id = 0
        else:
            reload_virtual_microbatch_id = None
        if (
            reload_virtual_microbatch_id is not None
            and reload_virtual_microbatch_id >= total_num_microbatches
        ):
            reload_virtual_microbatch_id = None
        return reload_virtual_microbatch_id

    def interleaved_offload_step(f_virtual_microbatch_id=None, b_virtual_microbatch_id=None):
        reload_virtual_microbatch_id = get_next_reload_virtual_microbatch_id(
            f_virtual_microbatch_id, b_virtual_microbatch_id
        )
        return offload_runtime.interleaved_step(
            forward_key=(
                get_offload_key(f_virtual_microbatch_id, forward=True)
                if f_virtual_microbatch_id is not None
                else None
            ),
            backward_key=(
                get_offload_key(b_virtual_microbatch_id, forward=False)
                if b_virtual_microbatch_id is not None
                else None
            ),
            reload_key=(
                get_offload_key(reload_virtual_microbatch_id, forward=False)
                if reload_virtual_microbatch_id is not None
                else None
            ),
            forward_enabled=should_offload_forward(f_virtual_microbatch_id),
            reload_enabled=should_reload_backward(reload_virtual_microbatch_id),
        )
    ...
    args = get_args()
    memory_modeling_enabled = bool(
        getattr(args, "bgl2_enable_memory_modeling", False)
    )
    dynamic_offload_enabled = (
        getattr(args, "activation_offload_threshold", None) is not None
        or getattr(args, "bgl2_activation_offload_threshold", None) is not None
    )
    activation_model_enabled = memory_modeling_enabled or dynamic_offload_enabled

    def _init_activation_memory_runtime(reporter=None):
        if activation_model_enabled:
            activation_modeling.initialize_activation_model(
                "moe_transformer", transformer_config=config
            )
        if reporter is None:
            reporter = MemoryReporter([0], enabled=memory_modeling_enabled)
        reporter.reset()
        reset_offload_manager(num_warmup_microbatches)
        return reporter

    memory_reporter = _init_activation_memory_runtime()

    global _PP_FAKE_FORWARD_BACKWARD_DONE
    should_run_fake_warmup = (
        not _PP_FAKE_FORWARD_BACKWARD_DONE
    ...
    if should_run_fake_warmup:
        real_data_iterator = data_iterator
        real_forward_only = forward_only
        real_grad_sync_func = config.grad_sync_func
        real_param_sync_func = config.param_sync_func
        real_bgl2_patch_te = getattr(args, "bgl2_patch_te", False)
        cuda_rng_tracker = None
        cuda_rng_tracker_states = None
        cpu_rng_state = None
        cuda_rng_state = None
        model_training_modes = None
        origin_vp_rank = parallel_state.get_virtual_pipeline_model_parallel_rank()
        try:
            if real_bgl2_patch_te:
                args.bgl2_patch_te = False

            data_iterator = _build_synthetic_data_iterators_like(real_data_iterator)
            forward_only = False
            config.grad_sync_func = None
            config.param_sync_func = None
            # Avoid TE fused-attention training/dropout cudnn plan failures during
            # the synthetic compute-only warmup; restore exact module modes below.
            model_training_modes = _set_model_chunks_training(model, False)
            ...
            try:
                ...
                cuda_rng_tracker_states = _clone_cuda_rng_tracker_states(
                    cuda_rng_tracker.get_states()
                )
            except Exception:
                cuda_rng_tracker = None

            fake_backward_virtual_microbatch_id = 0
            fake_model_chunk_id = get_model_chunk_id(
                fake_backward_virtual_microbatch_id, forward=False
            )
            fake_forward_virtual_microbatch_id = None
            for virtual_microbatch_id in range(total_num_microbatches):
                if get_model_chunk_id(virtual_microbatch_id, forward=True) == fake_model_chunk_id:
                    fake_forward_virtual_microbatch_id = virtual_microbatch_id
                    break
            if fake_forward_virtual_microbatch_id is None:
                raise RuntimeError(
                    "Failed to map fake forward virtual microbatch to fake backward chunk."
                )

            parallel_state.set_virtual_pipeline_model_parallel_rank(fake_model_chunk_id)
            input_tensors[fake_model_chunk_id].append(
                None
                if is_vp_first_stage(vp_stage=fake_model_chunk_id)
                else _build_local_fake_pp_input_tensor(
                    tensor_shape,
                    model[fake_model_chunk_id],
                    requires_grad=True,
                )
            )
            with torch.enable_grad():
                fake_forward_microbatch_id = get_microbatch_id_in_model_chunk(
                    fake_forward_virtual_microbatch_id,
                    forward=True,
                )
                output_tensor = forward_step_helper(
                    fake_forward_virtual_microbatch_id,
                    fake_forward_microbatch_id,
                    checkpoint_activations_microbatch=None,
                )
            if not is_vp_last_stage(vp_stage=fake_model_chunk_id):
                output_tensor_grads[fake_model_chunk_id].append(
                    _build_fake_output_grad_like(output_tensor)
                )
            _deallocate_output_tensor_tree(
                output_tensor,
                config.deallocate_pipeline_outputs,
            )
            backward_step_helper(fake_backward_virtual_microbatch_id)
            for model_chunk in model:
                for param in model_chunk.parameters():
                    param.grad = None
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            _PP_FAKE_FORWARD_BACKWARD_DONE = True
        finally:
            parallel_state.set_virtual_pipeline_model_parallel_rank(origin_vp_rank)
            data_iterator = real_data_iterator
            forward_only = real_forward_only
            config.grad_sync_func = real_grad_sync_func
            config.param_sync_func = real_param_sync_func
            if model_training_modes is not None:
                _restore_model_chunks_training(model, model_training_modes)
            if real_bgl2_patch_te:
                args.bgl2_patch_te = True
            forward_data_store.clear()
            total_num_tokens.zero_()
            synchronized_model_chunks.clear()
            for tensors in input_tensors:
                tensors.clear()
            for tensors in output_tensors:
                tensors.clear()
            if output_tensor_grads is not None:
                for tensors in output_tensor_grads:
                    tensors.clear()
            if cpu_rng_state is not None:
                torch.set_rng_state(cpu_rng_state)
            if cuda_rng_state is not None:
                torch.cuda.set_rng_state(cuda_rng_state)
            if cuda_rng_tracker is not None and cuda_rng_tracker_states is not None:
                cuda_rng_tracker.set_states(cuda_rng_tracker_states)
        # Synthetic compute can mutate offload bookkeeping.  Start the real
        # warmup measurement and manager state from a clean baseline.
        memory_reporter = _init_activation_memory_runtime(memory_reporter)
        print("[bgl2 fake pp warmup forward backward done]")
    ...
    for k in range(num_warmup_microbatches):
        memory_reporter.record()
        cur_model_chunk_id = get_model_chunk_id(k, forward=True)
        debug_log(
            "schedule_warmup_iter_begin",
            k=k,
            model_chunk_id=cur_model_chunk_id,
            recv_prev_wait_handles=len(recv_prev_wait_handles),
        )
        ...
        if config.overlap_p2p_comm_warmup_flush:
            if not is_vp_first_stage(vp_stage=cur_model_chunk_id) and k != 0:
                ...
                debug_log("schedule_warmup_recv_prev_wait_begin", k=k)
                recv_prev_wait_handle.wait()
                debug_log("schedule_warmup_recv_prev_wait_end", k=k)
        ...
        if config.overlap_p2p_comm_warmup_flush and not parallel_state.is_pipeline_first_stage(
            ...
            debug_log("schedule_warmup_prefetch_forward_begin", k=k, recv_prev=recv_prev)
            ...
            debug_log("schedule_warmup_prefetch_forward_end", k=k, recv_prev=recv_prev)
        ...
        debug_log(
            "schedule_warmup_forward_compute_begin",
            k=k,
            microbatch_id=microbatch_id,
            model_chunk_id=cur_model_chunk_id,
        )
        with interleaved_offload_step(f_virtual_microbatch_id=k):
            output_tensor = forward_step_helper(k, microbatch_id, checkpoint_activations_microbatch)
        debug_log("schedule_warmup_forward_compute_end", k=k)
        ...
        if not config.overlap_p2p_comm_warmup_flush:
            debug_log(
                "schedule_warmup_p2p_no_flush_begin",
                k=k,
                recv_prev=recv_prev,
            ...
            debug_log("schedule_warmup_p2p_no_flush_end", k=k, recv_prev=recv_prev)
            if recv_prev:
                input_tensors[next_forward_model_chunk_id].append(input_tensor)
            deallocate_output_tensor(output_tensor, config.deallocate_pipeline_outputs)
        else:
            debug_log("schedule_warmup_p2p_flush_begin", k=k, recv_prev=recv_prev)
            ...
            if send_next_wait_handle is not None:
                debug_log("schedule_warmup_send_next_wait_begin", k=k)
                send_next_wait_handle.wait()
                debug_log("schedule_warmup_send_next_wait_end", k=k)
            ...
            debug_log("schedule_warmup_p2p_flush_end", k=k, recv_prev=recv_prev)
        ...
        if config.overlap_p2p_comm:
            if (
                ...
                debug_log("schedule_warmup_initial_backward_p2p_begin", k=k, recv_next=recv_next)
                ...
                debug_log("schedule_warmup_initial_backward_p2p_end", k=k, recv_next=recv_next)
                if send_prev_wait_handle is not None:
                    debug_log("schedule_warmup_send_prev_wait_begin", k=k)
                    send_prev_wait_handle.wait()
                    debug_log("schedule_warmup_send_prev_wait_end", k=k)
        ...
        debug_log("schedule_warmup_iter_end", k=k)
    ...
    for k in range(num_microbatches_remaining):
        ...
        debug_log(
            "schedule_steady_iter_begin",
            k=k,
            forward_k=forward_k,
            forward_model_chunk_id=cur_model_chunk_id,
            microbatch_id=microbatch_id,
        )
        if config.overlap_p2p_comm:
            if not is_vp_first_stage(vp_stage=cur_model_chunk_id):
                if config.overlap_p2p_comm_warmup_flush:
                    ...
                    recv_prev_wait_handle = recv_prev_wait_handles.pop(0)
                    debug_log("schedule_steady_recv_prev_wait_begin", k=k, forward_k=forward_k)
                    recv_prev_wait_handle.wait()
                    debug_log("schedule_steady_recv_prev_wait_end", k=k, forward_k=forward_k)
                else:
            ...
            with interleaved_offload_step(
                ...
                debug_log("schedule_steady_forward_compute_end", k=k, forward_k=forward_k)
                ...
                # previous stage
                debug_log(
                    "schedule_steady_forward_p2p_begin",
                    k=k,
                    forward_k=forward_k,
                    recv_prev=recv_prev,
                )
                ...
                debug_log(
                    "schedule_steady_forward_p2p_end",
                    k=k,
                    forward_k=forward_k,
                    recv_prev=recv_prev,
                )
                if send_next_wait_handle is not None:
                    debug_log("schedule_steady_send_next_wait_begin", k=k, forward_k=forward_k)
                    send_next_wait_handle.wait()
                    debug_log("schedule_steady_send_next_wait_end", k=k, forward_k=forward_k)
                if fwd_wait_handles is not None:
                    send_next_wait_handle = (
                ...
                if not is_vp_last_stage(vp_stage=backward_model_chunk_id):
                    if config.overlap_p2p_comm_warmup_flush:
                        ...
                        if recv_next_wait_handles is not None and recv_next_wait_handles:
                            recv_next_wait_handle = recv_next_wait_handles.pop(0)
                            debug_log(
                                "schedule_steady_recv_next_wait_begin",
                                k=k,
                                backward_k=backward_k,
                            )
                            recv_next_wait_handle.wait()
                            debug_log(
                                "schedule_steady_recv_next_wait_end",
                                k=k,
                                backward_k=backward_k,
                            )

                debug_log("schedule_steady_backward_compute_begin", k=k, backward_k=backward_k)
                input_tensor_grad = backward_step_helper(backward_k)
                debug_log("schedule_steady_backward_compute_end", k=k, backward_k=backward_k)
            ...
            debug_log(
                "schedule_steady_backward_p2p_begin",
                k=k,
                backward_k=backward_k,
                recv_next=recv_next,
            )
            ...
            debug_log(
                "schedule_steady_backward_p2p_end",
                k=k,
                backward_k=backward_k,
                recv_next=recv_next,
            )
            if send_prev_wait_handle is not None:
                debug_log("schedule_steady_send_prev_wait_begin", k=k, backward_k=backward_k)
                send_prev_wait_handle.wait()
                debug_log("schedule_steady_send_prev_wait_end", k=k, backward_k=backward_k)
            if bwd_wait_handles is not None:
                send_prev_wait_handle = (
            ...
            backward_k = k
            debug_log("schedule_steady_no_overlap_compute_begin", k=k, forward_k=forward_k, backward_k=backward_k)
            with interleaved_offload_step(
                f_virtual_microbatch_id=forward_k,
                b_virtual_microbatch_id=backward_k,
            ):
                ...
                # Backward pass.
                input_tensor_grad = backward_step_helper(backward_k)
            debug_log("schedule_steady_no_overlap_compute_end", k=k, forward_k=forward_k, backward_k=backward_k)
            ...
            # Communicate tensors.
            debug_log(
                "schedule_steady_no_overlap_p2p_begin",
                k=k,
                forward_k=forward_k,
                backward_k=backward_k,
                recv_prev=recv_prev,
            ...
            debug_log("schedule_steady_no_overlap_p2p_end", k=k, forward_k=forward_k, backward_k=backward_k)
            deallocate_output_tensor(output_tensor, config.deallocate_pipeline_outputs)
        ...
        debug_log("schedule_steady_iter_end", k=k, forward_k=forward_k)
        memory_reporter.report()
    ...
    if not forward_only:
        if bwd_wait_handles is not None:
            for bwd_wait_handle in bwd_wait_handles.values():
                debug_log("schedule_cooldown_initial_bwd_wait_begin")
                bwd_wait_handle.wait()
                debug_log("schedule_cooldown_initial_bwd_wait_end")

        if are_all_microbatches_in_warmup:
            debug_log("schedule_cooldown_recv_backward_begin", curr_vp_stage=curr_vp_stage)
            ...
            debug_log("schedule_cooldown_recv_backward_end", curr_vp_stage=curr_vp_stage)
        for k in range(num_microbatches_remaining, total_num_microbatches):
            memory_reporter.record()
            cur_model_chunk_id = get_model_chunk_id(k, forward=False)
            debug_log("schedule_cooldown_iter_begin", k=k, model_chunk_id=cur_model_chunk_id)
            if not is_vp_last_stage(vp_stage=cur_model_chunk_id) and k != 0:
                if config.overlap_p2p_comm_warmup_flush:
                    ...
                    recv_next_wait_handle = recv_next_wait_handles.pop(0)
                    debug_log("schedule_cooldown_recv_next_wait_begin", k=k)
                    recv_next_wait_handle.wait()
                    debug_log("schedule_cooldown_recv_next_wait_end", k=k)
                else:
                    if recv_next_wait_handles is not None and recv_next_wait_handles:
                        recv_next_wait_handle = recv_next_wait_handles.pop(0)
                        debug_log("schedule_cooldown_recv_next_wait_begin", k=k)
            ...
            if config.overlap_p2p_comm_warmup_flush and not parallel_state.is_pipeline_last_stage(
                ...
                debug_log("schedule_cooldown_prefetch_backward_begin", k=k, recv_next=recv_next)
                ...
                debug_log("schedule_cooldown_prefetch_backward_end", k=k, recv_next=recv_next)

                if bwd_wait_recv_handles:
                    recv_next_wait_handles.append(bwd_wait_recv_handles.pop("recv_next"))

            debug_log("schedule_cooldown_backward_compute_begin", k=k)
            with interleaved_offload_step(b_virtual_microbatch_id=k):
                input_tensor_grad = backward_step_helper(k)
            debug_log("schedule_cooldown_backward_compute_end", k=k)
            ...
            if config.overlap_p2p_comm_warmup_flush:
                debug_log("schedule_cooldown_backward_p2p_flush_begin", k=k, recv_next=recv_next)
                ...
                debug_log("schedule_cooldown_backward_p2p_flush_end", k=k, recv_next=recv_next)

                if send_prev_wait_handle is not None:
                    debug_log("schedule_cooldown_send_prev_wait_begin", k=k)
                    send_prev_wait_handle.wait()
                    debug_log("schedule_cooldown_send_prev_wait_end", k=k)
                if bwd_wait_handles is not None:
                    send_prev_wait_handle = (
                        bwd_wait_handles.pop("send_prev")
                        if "send_prev" in bwd_wait_handles
                ...
                debug_log("schedule_cooldown_backward_p2p_begin", k=k, recv_next=recv_next)
                ...
                debug_log("schedule_cooldown_backward_p2p_end", k=k, recv_next=recv_next)

                if recv_next:
                    output_tensor_grads[next_backward_model_chunk_id].append(output_tensor_grad)
            debug_log("schedule_cooldown_iter_end", k=k)
            memory_reporter.report()

        if send_prev_wait_handle is not None:
            debug_log("schedule_cooldown_final_send_prev_wait_begin")
            send_prev_wait_handle.wait()
            debug_log("schedule_cooldown_final_send_prev_wait_end")
        ...
        if config.grad_sync_func is not None:
            for model_chunk_id in range(num_model_chunks):
                if model_chunk_id not in synchronized_model_chunks:
                    ...
                    synchronized_model_chunks.add(model_chunk_id)
    debug_log("schedule_flush_interleaved_begin")
    offload_runtime.flush_interleaved()
    debug_log("schedule_flush_interleaved_end")
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    if dynamic_offload_enabled or memory_modeling_enabled:
        activation_modeling.initialize_activation_model(
            "moe_transformer", transformer_config=config
        )
    reset_offload_manager(num_warmup_microbatches)
