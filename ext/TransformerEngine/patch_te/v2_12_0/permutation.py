...
class _moe_unpermute_index_map(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.tensor_pack = pack_hook(inp, op_name="permutation")
        ctx.save_for_backward(row_id_map, probs)
    ...
    def backward(
        ...
        inp = unpack_hook(ctx.tensor_pack)
        row_id_map, probs = ctx.saved_tensors
...
class _moe_unpermute_mask_map(torch.autograd.Function):
    ...
    def forward(
        ...
        if with_probs:
            ...
            ctx.tensor_pack = pack_hook(inp, op_name="permutation")
            ctx.save_for_backward(row_id_map, merging_probs, pad_offsets)
        else:
            ctx.save_for_backward(row_id_map, pad_offsets)
        ctx.num_experts = num_experts
        ctx.num_tokens = num_tokens
    ...
    def backward(ctx, unpermuted_act_grad):
        ...
        if ctx.needs_input_grad[0]:
            if ctx.with_probs:
                ...
                fwd_input = unpack_hook(ctx.tensor_pack)
                row_id_map, merging_probs, pad_offsets = ctx.saved_tensors
            else:
                row_id_map, pad_offsets = ctx.saved_tensors
